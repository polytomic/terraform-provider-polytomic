...
For detailed configuration guidance, see the [HTTP Enrichment connection guide](https://apidocs.polytomic.com/guides/configuring-your-connections/connections/httpenrichment).
...
- `id` (String, Read-only) HTTP Enrichment Connection identifier.
...
- `example_body` (String, Optional) Example body

    Example body to be saved with the query
- `example_inputs` (Map of String, Optional) Example inputs

    Example inputs to be saved with the connection
- `fields` (Attributes Set, Optional) List of fields to be returned by the enrichment See [below for nested schema](#nestedatt--configuration--fields).
...
- `input_mappings` (Attributes Set, Optional) Input mappings
...
- `method` (String, Required) HTTP Method Valid values: <code>GET</code>, <code>POST</code>.
...
- `path` (String, Optional) JSONPath expression to extract the field from the response
...
- `name` (String, Optional) Name of the input mapping
- `required` (Boolean, Optional) Whether the input mapping is required
- `type` (String, Optional) Type of the input mapping
