...
For detailed configuration guidance, see the [Azure SQL connection guide](https://apidocs.polytomic.com/guides/configuring-your-connections/connections/azuresql).
...
- `id` (String, Read-only) Azure SQL Connection identifier.
...
- `access_key` (String, Sensitive, Optional) Storage account access key (destination only)
- `account_name` (String, Optional) Storage account name (destination only)
- `blob_store` (Boolean, Optional) Use Azure blob storage for faster bulk loading (destination only)
- `container_name` (String, Optional) Storage container name (destination only)
